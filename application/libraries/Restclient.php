...
class Restclient{
    ...
    public function put($url, $parameters=array(), $headers=array()){
        //echo'<pre>'; echo print_r($parameters); exit;
    ...
    public function execute($url, $method='GET', $parameters=array(), $headers=array()){
        ...
        $client = clone $this;
        $client->url = $url;
        //echo'<pre>'; echo print_r($client->url); exit; 
        $client->handle = curl_init();
        $curlopt = array(
            // CURLOPT_HEADER => TRUE, not to get header in response.
        ...
        if(count($client->options['headers']) || count($headers)){
            ...
            if(isset($headers[0]) && $headers[0] == 'form-data')
            {
                unset($client->options['headers'][0]);
            }
            $headers = array_merge($client->options['headers'], $headers);
            foreach($headers as $key => $value){
                // $curlopt[CURLOPT_HTTPHEADER][] = sprintf("%s:%s", $key, $value); removed as header set isn't working in normal scenario.
        ...
        /*curl_setopt($client->handle, CURLOPT_HTTPHEADER, array('Authorization: ' . $tokenResponse));
        curl_setopt($client->handle, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);*/
        $client->response = curl_exec($client->handle);
        // $client->parse_response(curl_exec($client->handle)); removing as we have set header as false so that we wont be receiving header in response too.
